...
const Input = (args) => {
	...
	if (typeof args.prepend == "object" && !React.isValidElement(args.append)) {
	...
	return <div key={`${args.groupClassName}` + "-" + `${args.key}`} className={(typeof args.groupClassName != "undefined" || args.groupClassName != null) ? `${args.groupClassName}` : "form-group"}>
		{(typeof args.prelabel != "undefined" && args.prelabel != null) &&
			<label key={`prelabel-${args.id}`} htmlFor={args.id} className={(typeof args.type != "undefined") ? args.type == "checkbox" ? "form-check-label" : (args.type == "text" || args.type == "password" || args.type == "email") ? "sr-only" : args.type == "file" ? "file-label col" : undefined : undefined}>{args.prelabel}</label>}
		...
		{(typeof args.aplabel != "undefined" && args.aplabel != null) &&
			<label key={`prelabel-${args.id}`} htmlFor={args.id} className={(typeof args.type != "undefined") ? args.type == "checkbox" ? "form-check-label" : args.type == "text" ? "sr-only" : undefined : undefined}>{args.aplabel}</label>}
	</div>;
}
